...
create table if not exists public.profiles (
  ...
  business_name text,
  primary_goal text not null default 'maintain' check (primary_goal in ('lose_basic','lose_muscle','lose_gain_muscle','gain_basic','gain_muscle_maintain_fat','lean_bulk','maintain','recomp','lose','gain','health','custom','other')),
...
alter table public.profiles add column if not exists primary_goal text not null default 'maintain';
...
create table if not exists public.user_foods (
  ...
  conversion_mode text not null default 'none' check (conversion_mode in ('none','estimate')),
...
-- ================= VERIFIED TRAINERS (V44) =================
-- Trainer accounts may exist without verification, but trainer-only capabilities
-- require an approved verification record. This keeps role selection separate
-- from trust/privilege status.
...
create index if not exists trainer_verifications_status_requested_idx
  on public.trainer_verifications(status, requested_at desc);
...
create policy "trainer verification own read"
  on public.trainer_verifications for select to authenticated
  using (auth.uid() = user_id);
create policy "trainer verification own insert"
  on public.trainer_verifications for insert to authenticated
  with check (auth.uid() = user_id and exists (select 1 from public.profiles p where p.id=auth.uid() and p.role='trainer'));
create policy "trainer verification own update"
  on public.trainer_verifications for update to authenticated
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id and status in ('pending','rejected'));
create policy "trainer verification admin read"
  on public.trainer_verifications for select to authenticated
  using (public.is_admin());
create policy "trainer verification admin update"
  on public.trainer_verifications for update to authenticated
  using (public.is_admin()) with check (public.is_admin());

create or replace function public.is_verified_trainer(p_user_id uuid default auth.uid())
returns boolean language sql stable security definer set search_path=public as $$
  select exists (
    select 1
    from public.profiles p
    join public.trainer_verifications v on v.user_id=p.id
    where p.id=p_user_id and p.role='trainer' and v.status='approved'
  );
$$;
...
begin
  ...
  insert into public.trainer_verifications(user_id,status,requested_at,updated_at)
  values(auth.uid(),'pending',now(),now())
  on conflict (user_id) do update set status='pending', requested_at=now(), reviewed_at=null, reviewed_by=null, reviewer_note=null, updated_at=now()
  returning * into r;
  return r;
end;
$$;
...
create or replace function public.admin_list_trainer_verifications()
returns table(user_id uuid, display_name text, email text, business_name text, status text, requested_at timestamptz, reviewed_at timestamptz, reviewer_note text)
language plpgsql security definer set search_path=public as $$
begin
  if not public.is_admin() then raise exception 'Administrator access required.'; end if;
  return query
...
create or replace function public.admin_set_trainer_verification(p_user_id uuid, p_status text, p_note text default null)
returns public.trainer_verifications
language plpgsql security definer set search_path=public as $$
declare r public.trainer_verifications;
begin
  ...
  insert into public.trainer_verifications(user_id,status,requested_at,reviewed_at,reviewed_by,reviewer_note,updated_at)
  values(p_user_id,p_status,coalesce((select requested_at from public.trainer_verifications where user_id=p_user_id),now()),now(),auth.uid(),nullif(trim(p_note),''),now())
  on conflict (user_id) do update set status=excluded.status,reviewed_at=now(),reviewed_by=auth.uid(),reviewer_note=excluded.reviewer_note,updated_at=now()
  returning * into r;
  if p_status in ('rejected','revoked') then
    update public.trainer_profiles set is_public=false, updated_at=now() where user_id=p_user_id;
  end if;
  return r;
end;
$$;
...
-- Existing trainer directory/profile access is now limited to verified trainers.
-- PostgreSQL cannot change RETURNS TABLE / OUT-parameter types with CREATE OR REPLACE.
drop function if exists public.search_trainers(text,smallint,smallint);
...
-- Only verified trainers can send/receive trainer-directory connections.
create or replace function public.request_trainer_connection(p_trainer_id uuid)
returns public.friend_connections
language plpgsql security definer set search_path=public as $$
declare r public.friend_connections;
begin
  if auth.uid() is null then raise exception 'You must be signed in.'; end if;
  if exists (select 1 from public.profiles where id=auth.uid() and role='trainer') then raise exception 'Trainer accounts cannot send trainer-directory requests.'; end if;
  if not public.is_verified_trainer(p_trainer_id) then raise exception 'That trainer is not currently verified.'; end if;
  if not exists (select 1 from public.trainer_profiles where user_id=p_trainer_id and is_public=true) then raise exception 'That trainer is not currently listed.'; end if;
  insert into public.friend_connections(requester_id,addressee_id,status) values(auth.uid(),p_trainer_id,'pending') returning * into r;
  return r;
exception when unique_violation then raise exception 'A connection request already exists.';
end;
...
-- Make the trainer profile itself read-only to unverified trainers except for
-- their private draft; public listing is only possible after verification.
drop policy if exists "trainer profiles public read" on public.trainer_profiles;
create policy "trainer profiles public read" on public.trainer_profiles for select to authenticated
using (is_public=true and public.is_verified_trainer(user_id) and not public.is_limited_minor());
...
-- ==========================================================
-- V45 TRAINER VERIFICATION APPLICATION
-- Requires substantive evidence before admin approval.
-- ==========================================================
...
begin
  if auth.uid() is null then raise exception 'You must be signed in.'; end if;
  if not exists(select 1 from public.profiles where id=auth.uid() and role='trainer') then raise exception 'Only trainer accounts can request verification.'; end if;
  if exists(select 1 from public.trainer_verifications where user_id=auth.uid() and status='approved') then raise exception 'This trainer account is already verified.'; end if;
  ...
  insert into public.trainer_verifications(user_id,status,requested_at,reviewed_at,reviewed_by,reviewer_note,updated_at,experience,credentials,credential_number,proof_url,professional_background,statement)
  values(auth.uid(),'pending',now(),null,null,null,now(),trim(p_experience),trim(p_credentials),nullif(trim(p_credential_number),''),trim(p_proof_url),trim(p_professional_background),trim(p_statement))
...
-- ==========================================================
-- CONSOLIDATED CURRENT-STATE ADDITIONS
-- Integrated from the former recipe-sharing, trainer-verification-social,
-- and public-launch migrations. This file is now the authoritative fresh-install schema.
-- ==========================================================

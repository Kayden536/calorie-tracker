...
- Verified-trainer-only Community Food publishing
- Privacy-conscious product analytics and administrator-visible client error reporting
...
## V44 verified trainers and administrators

Trainer accounts are separate from trainer verification. Unverified trainer accounts retain normal-user access and cannot use trainer-only functionality or establish trainer/client connections. Trainers can request verification from Settings; administrators can approve, reject, or revoke verification from the Admin Moderation page. Approved trainers receive a visible Verified Trainer badge.

Normal users do not publish directly to Community Foods. Community Food publishing is enforced at the UI, RLS, and database RPC layers and is reserved for verified trainers. Users can still search and use published Community Foods.

Administrator status is represented in `profiles.is_admin` and is now surfaced in the application UI. See `ADMIN-SETUP.md` for assigning the initial administrator accounts.


## V45 verification application
Trainer verification now requires a substantive application: training experience, credentials, professional background, applicant statement, and a public verification link. Administrators review these details before approval.


## Public launch preparation (v56)
See `PUBLIC-LAUNCH-CHECKLIST.md` and run `supabase-public-launch-v56-migration.sql` before deploying this build. Do not include `.env`; copy `.env.example` to your deployment environment and rotate any secret that may have been exposed.


## Supabase SQL

Database setup is consolidated into `supabase-schema.sql` for new installations and `supabase-legacy-upgrade.sql` for existing databases built from older MacroSync/PulsePlate migrations. See `SUPABASE-SQL-GUIDE.md`.
